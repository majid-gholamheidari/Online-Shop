                ...
                <div class="box-body">
                    <form action="{{ route('admin.store.product.store') }}" method="POST" class="ajax-form">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="title" name="title" placeholder="عنوان" value="{{ $product->title }}">
                        </div>

                        <div class="form-group">
                            <label for="description">توضیحات</label>
                            <textarea class="form-control" id="description" name="description" rows="3" placeholder="توضیحات">{!! $product->description !!}</textarea>
                        </div>
                            ...
                            <select class="form-control select2" id="parent" name="category_id">
                                ...
                                @foreach($categories->where('level', 1) as $category)
                                    ...
                                    @foreach($categories->where('level', 2)->where('category_id', $category->id) as $categoryLevel2)
                                        <option @selected($categoryLevel2->id == $product->category_id) value="{{ $categoryLevel2->id }}">{{ $category->title . " > " . $categoryLevel2->title }}</option>
                                        @foreach($categories->where('level', 3)->where('category_id', $categoryLevel2->id) as $categoryLevel3)
                                            <option @selected($categoryLevel3->id == $product->category_id) value="{{ $categoryLevel3->id }}">{{ $category->title . " > " . $categoryLevel2->title . " > " . $categoryLevel3->title }}</option>
                                        @endforeach
                                    @endforeach
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-lg-6 col-sm-12">
                            ...
                            <select class="form-control select2" id="status" name="status">
                                ...
                                <option @selected($product->status == \Illuminate\Database\Eloquent\Model::StatusItems[1]) value="{{ \Illuminate\Database\Eloquent\Model::StatusItems[1] }}">فعال</option>
                                <option @selected($product->status == \Illuminate\Database\Eloquent\Model::StatusItems[0]) value="{{ \Illuminate\Database\Eloquent\Model::StatusItems[0] }}">غیر فعال
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="form-group col-lg-6 col-sm-12">
                            ...
                            <input type="number" name="stock" id="stock" class="form-control" value="{{ $product->stock > 0 ? $product->stock : 0 }}">
                        </div>
                        ...
                        <div class="form-group col-lg-6">
                            ...
                            <input type="number" class="form-control" id="price" name="discount_price" placeholder="مبلغ تخفیف" value="{{ $product->discount_price }}">
                        </div>

                        <div class="form-group">
                            <input type="checkbox" class="custom-control-input" id="special_offer" {{ $product->special_offer == 'active' ? 'checked' : '' }} onchange="toggleSpecialOfferDateInput()">
...
@section('js')
    ...
    <script>
        ...
        let productBrand = @json($product->brand);
        if ($('#brand_id').find(`option[value='${productBrand.id}']`).length) {
            $('#brand_id').val(data.id).trigger('change');
        } else {
            let selectedBrandOption = new Option(productBrand.title + ` (${productBrand.en_title})`, productBrand.id, true, true);
            $('#brand_id').append(selectedBrandOption).trigger('change');
        }
